...
import { Film, Heart, Home, LogIn, Menu, Search, Ticket, UserRound, X } from 'lucide-react';
  ...
  return <>
    <motion.header initial={{ y: -90 }} animate={{ y: 0 }} className={`cinema-nav ${scrolled ? 'cinema-nav--solid' : ''}`} onMouseLeave={() => setHovered(null)}>
      ...
      <div className="cinema-nav__actions">
        <button onClick={() => setSearchOpen(true)} aria-label="Buscar películas, personas o géneros"><Search size={18} /></button>
        <Link to="/cuenta" aria-label="Colección y ver más tarde" className="relative"><Heart size={18} />{favorites.length + watchlist.length > 0 && <span className="nav-badge">{favorites.length + watchlist.length}</span>}</Link>
        ...
        <Link to="/cartelera" className="cinema-ticket-cta"><Ticket size={15} /><span>COMPRAR ENTRADAS</span></Link>
      </div>
    ...
    <nav className="cinema-mobile-dock" aria-label="Accesos rápidos">
      ...
      <NavLink to="/cuenta"><Heart size={18} /><span>Mi lista</span>{favorites.length + watchlist.length > 0 && <b>{favorites.length + watchlist.length}</b>}</NavLink>
    </nav>
